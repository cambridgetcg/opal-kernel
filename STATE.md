...
build: see heartbeat
health: active
last-commit: ddf69fa heartbeat: STATE.md sync after M7 runtime board init beat
uncommitted: 22
...
- interactive monitor with commands: help, brk, svc, unaligned, translate, walk, mapdev, guard, wx, noexec, low, abort, tick, ticks, ticktest, dtb, tree, fdtcons, el0, el0fault, tasks, spawn2, preempt, ipc, blkipc, sendblk, faultkill, sleep, wait
...
- M7: Apple Silicon bring-up via m1n1 — IN PROGRESS (EL2→EL1 drop in boot stub ✅, FDT-driven console discovery via compatible ✅, s5l UART driver 139 lines ✅, security audit: integer overflow fix in FDT parser + user VA validation in syscall handler ✅, FIQ handler upgraded from fatal `-> !` to recoverable timer-aware handler ✅, board/apple.rs skeleton with s5l UART instantiation + init scaffolding ✅, AIC interrupt controller driver 341 lines ✅, FDT find_by_compatible tree-wide search ✅, board/apple.rs init() now discovers AIC from FDT and brings it online ✅, runtime board selection via root compatible ✅, AIC event dispatch wiring in FIQ vector ✅, arm64 Image header (64-byte boot protocol header for m1n1) ✅, AIC MMIO base now translated via phys_to_virt before MMIO access ✅, flat Image build automation (Makefile: make image/run-img/inspect) ✅, FDT-from-x0 in kmain — kernel prefers the Linux boot protocol FDT pointer over RAM base, verified under QEMU Image boot (x0=0x48000000) ✅, load-PA discovery via adr _image_start ✅, genuinely position-independent boot: relocation delta (x23 = load_pa - link_pa) applied to every literal-pool address in boot stub + passed to table builder ✅, runtime-base s5l UART console (RuntimeS5lUart wrapper) + Apple board console wiring (Console=RuntimeS5lUart, init() discovers UART from FDT, console() constructor) ✅, dedicated Device-attribute page mapping for FDT-discovered MMIO (ioremap_device) with runtime table pool + monitor mapdev command ✅, framebuffer text console (hal/fb.rs — 8x16 bitmap font, FDT simple-framebuffer discovery, Normal-NC MAIR slot 2, ioremap_framebuffer, scroll, cursor, fmt::Write) ✅, runtime board init wired: kmain calls board::apple::init() on Apple boards and board::virt::init() on QEMU, console_print routes through board-selected Console enum ✅. Remaining: timers-over-FIQ on real hardware)
